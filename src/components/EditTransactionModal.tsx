...
const EditTransactionModal: React.FC<EditTransactionModalProps> = ({ transaction, onClose, onSaveSuccess }) => {
    ...
    const handlePreSave = () => {
        ...
        const categoryChanged = category !== transaction.category || subcategory !== (transaction.subcategory || '');
        if (transaction.predictedExpenseId && categoryChanged) {
    ...
    const handleConfirmUnlink = (unlink: boolean) => {
        if (!pendingChanges) return;
        
        const finalUpdates = { ...pendingChanges };
        if (unlink) {
            finalUpdates.predictedExpenseId = null;
        } else {
            // Se não desvincular, adiciona aviso nas notas
            const noteSuffix = ' (*categoria alterada)';
            if (!notes.includes(noteSuffix)) {
                finalUpdates.notes = notes + noteSuffix;
            }
        }
        
        setPendingChanges(finalUpdates);
