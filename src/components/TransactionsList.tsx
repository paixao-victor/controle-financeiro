...
import { getTransactionName, capitalize, removeAccents } from '@/utils/formatters';
...
const TransactionsList: React.FC<TransactionsListProps> = ({ searchQuery = '' }) => {
    ...
    const [filterPeriod, setFilterPeriod] = useState<'today' | 'last5' | 'this_month' | 'last_30' | 'last_60' | 'custom'>('last5');
    ...
    const summary = useMemo(() => {
        ...
        return { income, expense, total, savingsPercent, savingsAmount };
    }, [transactions]);
    ...
    return (
        <div className="flex flex-col h-full bg-background overflow-hidden" ref={listDragRef}>
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:p-8 pt-6 pb-32 md:pb-8 bg-background">
                        ...
                        <div className="md:col-span-2 relative overflow-hidden rounded-3xl bg-surface p-6 lg:p-8 shadow-xl group border border-gray-100 dark:border-white/5">
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between">
                                <div>
                                    ...
                                    <h3 className="text-content text-3xl lg:text-4xl font-extrabold tracking-tight">{formatCurrency(summary.total)}</h3>
                                </div>
                                <div className="flex md:hidden items-center justify-between mt-8 border-t border-gray-100 dark:border-white/10 pt-4">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-content text-lg font-bold tracking-tight">{formatCurrency(summary.income)}</span>
                                    </div>
                                    <div className="flex flex-col text-right">
                                        <span className="text-[10px] text-red-500 uppercase font-bold tracking-widest">Saídas</span>
                                        <span className="text-content text-lg font-bold tracking-tight">{formatCurrency(summary.expense)}</span>
                                    </div>
                                </div>
                                <div className="hidden md:flex items-center gap-6 mt-6">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-content text-sm font-black">{formatCurrency(summary.income)}</span>
                                    </div>
                                    <div className="flex flex-col border-l border-gray-100 dark:border-white/10 pl-6">
                                        ...
                                        <span className="text-content text-sm font-black">{formatCurrency(summary.expense)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-12 gap-6 lg:gap-8">
                        <div className="col-span-12 lg:col-span-8 space-y-6">
                            ...
                            <div ref={observerTarget} className="py-6 flex justify-center w-full">
                                {filterPeriod !== 'last5' && filterPeriod !== 'today' && visibleCount < (searchQuery ? transactions.length : transactions.length) && (
                    ...
                    <div className="space-y-4 py-4">
                        ...
                        <div className="bg-primary/5 dark:bg-primary/10 rounded-xl p-4">
                            <p className="text-xs text-content leading-relaxed">
                                ...
                                das suas receitas este mês.
                                {summary.savingsPercent >= 0 
                                    ? ' Continue assim para manter suas finanças saudáveis!' 
                                    : ' Considere reduzir despesas ou aumentar receitas.'}
                            </p>
                        </div>
                    </div>
                    <DialogFooter>
                        <Button onClick={() => setSavingsDetailOpen(false)}>Fechar</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
